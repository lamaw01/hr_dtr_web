...
if($_SERVER['REQUEST_METHOD'] == 'POST' && array_key_exists('employee_id', $input)){
    ...
    DATE_FORMAT(selfie_timestamp, '%Y-%m-%d') selfie_timestamp 
    ...
    AND tbl_employee.last_name IS NOT NULL AND tbl_logs.selfie_timestamp BETWEEN :date_from AND :date_to GROUP BY DATE_FORMAT(tbl_logs.selfie_timestamp, '%Y-%m-%d') ORDER BY tbl_logs.id ASC;";
    ...
    LEFT JOIN tbl_week_schedule ON tbl_employee.week_sched_id = tbl_week_schedule.week_sched_id 
    WHERE (tbl_logs.employee_id LIKE :employee_id OR tbl_employee.first_name LIKE :employee_id OR tbl_employee.last_name LIKE :employee_id) 
    AND tbl_employee.last_name IS NOT NULL AND tbl_logs.selfie_timestamp BETWEEN :date_from AND :date_to AND tbl_employee_department.department_id = :department 
    GROUP BY DATE_FORMAT(tbl_logs.selfie_timestamp, '%Y-%m-%d') ORDER BY tbl_logs.id ASC;";
